...
constexpr const char* ShenandoahGCPhaseTimes::GCMergeRSWorkItemsStrings[];

ShenandoahGCPhaseTimes::ShenandoahGCPhaseTimes(uint max_gc_threads) :
...
void ShenandoahGCPhaseTimes::reset() {
...
void ShenandoahGCPhaseTimes::record_gc_pause_start() {
...
double ShenandoahGCPhaseTimes::worker_time(GCParPhases phase, uint worker) {
...
void ShenandoahGCPhaseTimes::record_gc_pause_end() {
...
void ShenandoahGCPhaseTimes::record_time_secs(GCParPhases phase, uint worker_id, double secs) {
  _gc_par_phases[phase]->set(worker_id, secs);
}

// add a number of seconds to a phase
void ShenandoahGCPhaseTimes::add_time_secs(GCParPhases phase, uint worker_id, double secs) {
  _gc_par_phases[phase]->add(worker_id, secs);
}

void ShenandoahGCPhaseTimes::record_or_add_time_secs(GCParPhases phase, uint worker_id, double secs) {
  _gc_par_phases[phase]->set_or_add(worker_id, secs);
}

double ShenandoahGCPhaseTimes::get_time_secs(GCParPhases phase, uint worker_id) {
  return _gc_par_phases[phase]->get(worker_id);
}

void ShenandoahGCPhaseTimes::record_thread_work_item(GCParPhases phase, uint worker_id, size_t count, uint index) {
  _gc_par_phases[phase]->set_thread_work_item(worker_id, count, index);
}

void ShenandoahGCPhaseTimes::record_or_add_thread_work_item(GCParPhases phase, uint worker_id, size_t count, uint index) {
  _gc_par_phases[phase]->set_or_add_thread_work_item(worker_id, count, index);
}

size_t ShenandoahGCPhaseTimes::get_thread_work_item(GCParPhases phase, uint worker_id, uint index) {
  return _gc_par_phases[phase]->get_thread_work_item(worker_id, index);
}

// return the average time for a phase in milliseconds
double ShenandoahGCPhaseTimes::average_time_ms(GCParPhases phase) const {
  if (_gc_par_phases[phase] == nullptr) {
    return 0.0;
  }
...
size_t ShenandoahGCPhaseTimes::sum_thread_work_items(GCParPhases phase, uint index) {
  if (_gc_par_phases[phase] == nullptr) {
    return 0;
  }
...
size_t ShenandoahGCPhaseTimes::avg_thread_work_items(GCParPhases phase, uint index) {
  if (_gc_par_phases[phase] == nullptr) {
    return 0;
  }
  assert(_gc_par_phases[phase]->thread_work_items(index) != nullptr, "No sub count");
...
template <class T>
void ShenandoahGCPhaseTimes::details(T* phase, uint indent_level) const {
...
void ShenandoahGCPhaseTimes::print_thread_work_items(WorkerDataArray<double>* phase, uint indent_level, outputStream* out) const {
...
void ShenandoahGCPhaseTimes::debug_phase_merge_remset() const {
...
void ShenandoahGCPhaseTimes::log_phase(WorkerDataArray<double>* phase, uint indent_level, outputStream* out, bool print_sum) const {
...
void ShenandoahGCPhaseTimes::debug_phase(WorkerDataArray<double>* phase, uint extra_indent) const {
  LogTarget(Debug, gc, phases) lt;
  if (lt.is_enabled()) {
    ResourceMark rm;
    LogStream ls(lt);
...
void ShenandoahGCPhaseTimes::trace_phase(WorkerDataArray<double>* phase, bool print_sum, uint extra_indent) const {
...
void ShenandoahGCPhaseTimes::info_time(const char* name, double value) const {
  log_info(gc, phases)("  %s: " TIME_FORMAT, name, value);
}

void ShenandoahGCPhaseTimes::debug_time(const char* name, double value) const {
  log_debug(gc, phases)("    %s: " TIME_FORMAT, name, value);
}

void ShenandoahGCPhaseTimes::debug_time_for_reference(const char* name, double value) const {
  LogTarget(Debug, gc, phases) lt;
...
void ShenandoahGCPhaseTimes::trace_time(const char* name, double value) const {
  log_trace(gc, phases)("      %s: " TIME_FORMAT, name, value);
}

void ShenandoahGCPhaseTimes::trace_count(const char* name, size_t value) const {
  log_trace(gc, phases)("      %s: " SIZE_FORMAT, name, value);
}

double ShenandoahGCPhaseTimes::print_pre_evacuate_collection_set() const {
...
double ShenandoahGCPhaseTimes::print_evacuate_optional_collection_set() const {
...
double ShenandoahGCPhaseTimes::print_evacuate_initial_collection_set() const {
...
double ShenandoahGCPhaseTimes::print_post_evacuate_collection_set(bool evacuation_failed) const {
...
//   if (ShenandoahHeap::heap()->should_sample_collection_set_candidates()) {
...
void ShenandoahGCPhaseTimes::print_other(double accounted_ms) const {
  info_time("Other", _gc_pause_time_ms - accounted_ms);
}

void ShenandoahGCPhaseTimes::print(bool evacuation_failed) {
...
const char* ShenandoahGCPhaseTimes::phase_name(GCParPhases phase) {
  ShenandoahGCPhaseTimes* phase_times = ShenandoahHeap::heap()->phase_times();
